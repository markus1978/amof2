...
public class AsAnalysisEnvironment {
	...
	public AsAnalysisEnvironment(Extent e, Repository repo) {
		this.m2Extent = e;		
	...
	public void checkOclConstraint(String expression, NamedElement context, Type requiredType, boolean isCollection, boolean isUnique, boolean isOrdered) throws AsSemanticException {
		...
		try {
			...
			throw new AsSemanticException(ex);
		}
				...
				for (Object o2: ccd.getConstraint()) {
					...
					if (exprType instanceof MofOclModelElementTypeImpl || exprType instanceof MofEnumerationImpl || exprType instanceof PrimitiveImpl) {
						...
						if (!(!(context instanceof MultiplicityElement) || ((MultiplicityElement)context).getUpper() == 1)) {
							throw new AsSemanticException(errorPrefix + "has collection type for non collection feature.");							
						}
					} else if (exprType instanceof CollectionType) {
						if (!isCollection) {
							throw new AsSemanticException(errorPrefix + "has non collection type for collection feature.");						
						}
						exprClassifierType = ((CollectionType)exprType).getBaseElementType();
						if (exprType instanceof BagType) {
							if (isOrdered) {
								throw new AsSemanticException(errorPrefix + "has non ordered collection type for an ordered feature.");								
							}
							if (isUnique) {
								throw new AsSemanticException(errorPrefix + "has non unique collection type for an unique feature.");								
							}
						...
						throw new AsSemanticException(errorPrefix + "has unknown static type.");
					}
					...
					if (exprClassifierType instanceof MofOclModelElementTypeImpl || exprClassifierType instanceof MofEnumerationImpl) {
						...
						if (!requiredType.equals(mofDelegate)) {
							throw new AsSemanticException(errorPrefix + "has wrong type or base type.");
						}											
						...
						throw new AsSemanticException(errorPrefix + "has wrong base type.");
					}
				}
			}
		}
	...
	public void addAdditionalContextAttribute(ContextExtensionPin contextPin, Object value, Type attributeType, Type contextType) {
		addAdditionalContextAttribute(contextPin.getExtensionName(), value, attributeType, contextType);		
	}
	...
	public void addAdditionalContextAttribute(String name, Object value, Type attributeType, Type contextType) {
		...
		if (contextOclModelElementType == null) {
			throw new AsSemanticException("Cannot resolve context (" + contextName + ") of action " + this.toString());
		}
	...
	public void removeAdditionalAttribute() {
		if (additionalContext) {
			...
			MofOclModelElementTypeImpl contextOclModelElementType = (MofOclModelElementTypeImpl)oclEnvironment.lookupPathName(contextName);
			if (contextOclModelElementType == null) {
				throw new AsSemanticException("Cannot resolve context of action " + this.toString());
			}			
