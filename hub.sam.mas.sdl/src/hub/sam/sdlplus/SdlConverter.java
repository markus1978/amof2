 ...
 */
public class SdlConverter {
    private static SdlConverter theCompiler = new SdlConverter();
    private static final Trace theTrace = new Trace();

    /**
     * Returns a default compiler instance.
     */
    public static SdlConverter getCompiler() {
        return theCompiler;
    }
    ...
    private void initializeRepository() throws Exception {    	
        ...
        cmof.Package m3 = (cmof.Package)repository.getExtent(Repository.CMOF_EXTENT_NAME).query("Package:cmof"); 
        sdlMetaExtent = repository.createExtent("sdl-meta-extent", m3);
        repository.loadMagicDrawXmiIntoExtent(sdlMetaExtent, m3, "resources/models/sdl.mdxml");
        sdlPackage = (cmof.Package)sdlMetaExtent.query("Package:SDL");
        
        cmofFactory factory = sdlMetaExtent.getAdaptor(cmofFactory.class);
        Tag nsPrefixTag = factory.createTag();
        nsPrefixTag.setName(CMOFToXmi.NS_PREFIX_TAG_NAME);
        nsPrefixTag.setValue("sdl");
        sdlPackage.getTag().add(nsPrefixTag);        
    ...
    public String getContextInformation(cmof.reflection.Object context) {
        return context.getMetaClass() + ":" + context.toString() + " at " + SdlConverter.getCompiler().getContextM1(context);
    }
    ...
    public synchronized void compile(String[] args) {
        ...
        try {
            ...
            if (success) {
               ...
               repository.writeExtentToXmi(args[0] + ".xmi", sdlPackage, sdlModelExtent);
            }
    ...
    public static void main(String[] args) {
        SdlConverter.getCompiler().compile(args);
    }
}
