...
public class TupleTypeImpl extends DataTypeImpl implements TupleType {
	...
	public List getNames(){
	    Vector v = new Vector();
		Iterator i = getPartType().iterator();
		while(i.hasNext()) {
	...
	public TupleType project(Set nameSet) {
        ...
        return processor.getTypeFactory().buildTupleType((String[])names.toArray(new String[]{}), (Classifier[])types.toArray(new Classifier[]{}));
	}
    public TupleType join(TupleType t2) {
	    List names = new Vector(getNames());
	    List types = new Vector(getTypes());
    ...
    class PropertyPartImpl implements Property {
    ...
    public Boolean conformsTo(Classifier t2) {
        ...
        if (t2 instanceof TupleType) {
            ...
            while (i1.hasNext()) {
				...
				VariableDeclaration var2 = getVar(var1.getName(), l2); //(VariableDeclaration)i2.next();
				//var2 is null if no var in l2 found with same name as var1
				if (var2==null) return Boolean.FALSE;
                //--- Check type
