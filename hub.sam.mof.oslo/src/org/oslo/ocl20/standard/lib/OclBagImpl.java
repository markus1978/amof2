...
public class OclBagImpl
 ...
 implements OclBag
{
	protected OclBagImpl(Classifier eT, Object[] array, StdLibAdapter adapter) {
		super(eT,adapter);
	...
	public OclBoolean equalTo(OclBag bag2) {
		...
		while (it.hasNext()) {
			...
			if (!((Boolean)count(ob).equalTo(bag2.count(ob)).asJavaObject()).booleanValue())
				return adapter.Boolean(false);
		}
	...
	public OclBag intersection(OclBag bag2) {
		...
		while (i1.hasNext()) {
			...
			if (((Boolean)bag2.includes(o).asJavaObject()).booleanValue()) {
				b = b.including(o);
				i1.remove();
				b2.remove(o);
			}
