...
//              ok
	...
	public OclSequence asSequence() {
		// HUB changes -- non determinism
		Collection values = implementation();
		int size = values.size();
		Object[] randomSortedValues = new Object[size];
		
		int start = random.nextInt(size);
		Iterator valuesIt = values.iterator();
		for (int i = 0; i < size; i++) {
			randomSortedValues[(start + i) % size] = valuesIt.next();
		}
		return adapter.Sequence(this.getElementType(), randomSortedValues);
	}
